...
10 x 3 = 30</code></pre>`,
...

        {
          ...
          code: `def prix_remise(prix, remise):\n    return prix - prix * remise / 100\n\nprint(prix_remise(20, 100))`,
    ...
    return resultat</code></pre>
...

        {
          ...
          indices: [
            ...
            "« a est multiple de b » se traduit par : le reste de la division de a par b est nul. Renvoie cette comparaison.",
          ],
        },

        {
          ...
          indices: [
            ...
            "Il suffit donc de multiplier ce quotient par <code>a</code> pour retomber sur un multiple de <code>a</code>.",
            "Une seule ligne : le quotient entier de <code>b</code> par <code>a</code>, multiplié par <code>a</code>. Attention aux parenthèses.",
          ],
        },

        {
          ...
          indices: [
            ...
            "La formule est écrite dans l'énoncé : recopie-la en remplaçant chaque coordonnée par le paramètre correspondant.",
            "Les points sont alignés quand ce déterminant est nul : renvoie cette comparaison.",
          ],
        },
    ...
    /* ============================== APPLICATION ============================== */
    {
      ...
      etapes: [

        {
          ...
          indices: [
            "La remise, c'est le pourcentage de ce prix : multiplier par le pourcentage, puis diviser par 100.",
            "Le prix remisé est le prix de départ <em>moins</em> cette remise. Une seule ligne, avec <code>return</code>.",
          ],
        },

        {
          ...
          indices: [
            "Le carré de la taille s'écrit <code>taille * taille</code>, ou avec l'exposant <code>** 2</code>.",
            "Divise le poids par ce carré — et encadre le dénominateur de parenthèses, sinon Python divise par la taille puis remultiplie par la taille.",
          ],
        },
...

        {
          ...
          indices: [
            "Appliquer 30 % de remise, c'est retirer trente centièmes du prix — ou, ce qui revient au même, n'en garder que 0,7.",
            "La seconde ligne manquante renvoie le tarif. Attention à son décalage : dans la fonction, mais en dehors du <code>if</code>, sinon les clients sans carte ne recevraient rien.",
          ],
        },

        {
          ...
          indices: [
            "Le coefficient est un quotient : la différence des ordonnées au numérateur, celle des abscisses au dénominateur.",
            "Chacune des deux différences doit être entre parenthèses, sinon la priorité des opérations fausse le calcul.",
          ],
        },
...
7 x 6 = 42</code></pre>`,
          ...
          indices: [
            "Ajoute deux paramètres à la définition, après <code>n</code> : un pour le début, un pour la fin.",
            "Dans la boucle, remplace les deux bornes fixes par ces paramètres — sans oublier que celle de droite est exclue.",
            "L'appel doit maintenant transmettre trois valeurs, dans l'ordre de la définition.",
          ],
        },
    ...
    /* ================================ DÉFIS ================================ */
    {
      ...
      etapes: [

        {
          ...
          indices: [
            "Range d'abord les deux écarts dans des variables : l'écart des abscisses, celui des ordonnées.",
            "La distance est la racine carrée de la somme de leurs carrés. Encadre bien toute la somme avant d'y appliquer l'exposant 0.5.",
          ],
        },

        {
          ...
          indices: [
            "Commence par récupérer le coefficient et l'ordonnée à l'origine dans deux variables, en appelant les fonctions déjà écrites.",
            "Le point est sur la droite quand son ordonnée est égale à ce que donne l'équation pour son abscisse. Renvoie cette comparaison.",
          ],
        },
...

        {
          ...
          indices: [
            "L'écart d'une valeur à la moyenne, c'est <code>v - m</code> ; son carré s'obtient en la multipliant par elle-même.",
            "Première ligne à compléter : ajoute ce carré à <code>total</code>, comme pour n'importe quel accumulateur.",
            "Seconde ligne : divise <code>total</code> par le nombre de valeurs, puis prends la racine carrée du tout — attention aux parenthèses avant l'exposant 0.5.",
          ],
        },
